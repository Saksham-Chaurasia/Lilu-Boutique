...
def save_blouse_data(submitted_details):
    category = 'Blouse'
    ...
    if submitted_details["Files"]:
        for uploaded_file in submitted_details["Files"]:
            ...
            composite_file_name = f"{customer_id}-{blouse_id}-{category}-{file_name}"
    ...
    return blouse_id, pdf_file_name, customer_id


    
def view_all_blouse_data():
    conn = connection_database.db()
    cursor = conn.cursor()
    ...
    query = """SELECT cd.customer_id, cd.name, cd.phone, cd.email, cd.dates, cd.delivery_date,cd.category, cd.registration_date,
                      ...
                      bd.padding, bd.piping, bd.lace, bd.tassels, bd.comments,bf.file_name
               FROM customer_data cd
               JOIN blouse_data bd ON cd.customer_id = bd.customer_id
               LEFT JOIN blouse_files bf ON bd.blouse_id = bf.blouse_id
               where cd.category ='Blouse'
               """
